...
function getNames(presidentsArr) {
  return presidentsArr.map((obj) => obj.name);
}

// Iteration 2 | Democratic Presidents - `filter()`
function getDemocraticPresidents(presidentsArr) {
  return presidentsArr.filter((obj) => obj.party === "Democratic");
}
...
function countYearsInOffice(presidentsArr) {
  return presidentsArr.reduce((acc, obj) => {
    if (obj.leftOffice > 0) {
      return acc + (obj.leftOffice - obj.tookOffice);
    }
    return acc;
  }, 0);
}
...
function getAgeAtInauguration(presidentsArr) {
  return presidentsArr.map((obj) => {
    obj.ageAtInauguration = obj.tookOffice - obj.birthYear;
    return obj;
  });
}
...
function getPresidentsBornAfter(presidentsArr, year) {
  return presidentsArr.filter((obj) => obj.birthYear > year);
}
